...
// "off"===g.stopLoop&&a.revresume();a=g}function L(){w&&!N||M();N=!0}function O(){window.removeEventListener("resize",O);clearTimeout(C);q=!0;a.off("revolution.slide.onloaded",y).off("revolution.slide.onafterswap",B);punchgs.TweenLite.killTweensOf(p(".rs_addon_revealer").find("*"));x&&(x.eventCallback("onUpdate"),x.kill(),x);M()}function P(){f=a.find(".tp-loader");f.length?Q(f):window.requestAnimationFrame(P)}function Q(b){b&&b.length?b[0].className="tp-loader":b=p('<div class="tp-loader" />').appendTo(a);
